    ...
    if sampling_on == False:
        ...
        if local_total >= pacing_vol:
            if trigger_sampler():
                ...
                if g_volume_total >= pacing_vol/5:
                    print("Flow too high for pacing!!")
                    print("Sampling every 5 minutes or less...")

                if g_volume_total >= pacing_vol:
                    incr_missed_samples = g_volume_total / pacing_vol #remaining incremental volume divided by pacing = missed samples
                    missed_samples = missed_samples + 1.
                    print ("Flow too high for pacing!!")
                    print ("Missed samples per increment: "+ float("%.2f%"%incr_missed_samples))
                    print ("Missed samples: " + float("%.2f%"%missed_samples))

                    # Write a log entry for missed samples
                    reading = Reading(label="MissedSample", time=time_scheduled(),
                                      etype='E', value=missed_samples, quality='G')
                    reading.write_log()
                    ## reset volume_total
                    g_volume_total = 0.
